...
def reconocer():
    try:
        ...
        # [COMENTADO] Reconocimiento facial desactivado por el momento
        # from reconocer_usuario import procesar_imagen
        # resultado = procesar_imagen(data['imagen'], dni)
        
        # Por ahora, aceptar la imagen sin reconocimiento facial
        resultado = {
            "success": True,
            "nombre": session['voto_actual'].get('nombre', 'Usuario'),
            "mensaje": "Imagen recibida (reconocimiento facial desactivado)"
        }

        if resultado['success']:
            # Actualizar la sesión con el nombre
            session['voto_actual']['nombre_reconocido'] = resultado['nombre']
            session.modified = True
            print(f"Procesamiento de imagen para DNI {dni}: {resultado['nombre']}")
        else:
            print(f"Error en procesamiento para DNI {dni}: {resultado.get('error', 'Desconocido')}")
...
if __name__ == '__main__':
    ...
    print("La aplicación Flask se ejecutará en el puerto 5002")
